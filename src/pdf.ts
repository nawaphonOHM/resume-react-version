...
function buildResumeLines(resume: ResumeProfile) {
  ...
  for (const item of resume.experience) {
    ...
    lines.push(`${item.period} | ${item.location} | ${item.employmentTypes.join(', ')}`)
  ...
  lines.push(`${resume.education.institution} | ${resume.education.period} | GPAX ${resume.education.gpax}`)
  lines.push(`Senior project: ${resume.education.seniorProject.name} (${resume.education.seniorProject.url})`)
  lines.push('')
...
function buildPdf(resume: ResumeProfile) {
  ...
  objects.push(`<< /Type /Pages /Count ${lineGroups.length} /Kids [${pageObjectIds.map((id) => `${id} 0 R`).join(' ')}] >>`)
